  ...
  COMMAND LINE: "web/ftangle web/t_type -A -# --F -= 1.53/web/t_type.h" \
  RUN TIME: "Saturday, September 23, 1995 at 16:17." \
  WEB FILE:    "web/t_type.web" \
  CHANGE FILE: (none)
#endif
#define OUT_CHAR(c)out_char((eight_bits)(c))/* Cast avoids warning messages. */ \



#include "s_type.h" /* Prototypes for \.{style.web}. */
#include "r_type.h" /* Prototypes for \.{ratfor.web}. */
#include "e_type.h" /* Prototypes for \.{eval.web}. */
#include "m_type.h" /* Prototypes for \.{macs.web}. */
...
SRTN see_tstatistics PROTO((VOID));
